...
<div class="page-shell">
    ...
    <div class="page-header mb-4">
        <h1>Edit Soal</h1>
        <p>Perbarui soal untuk assessment: <strong>{{ $assessment->title }}</strong></p>
    </div>

    <div class="glass-card-strong p-4 p-lg-5">
        <form action="{{ route('guru.assessment.questions.update', [$assessment->id, $question->id]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    ...
                    <input type="text" name="option_a" class="form-control @error('option_a') is-invalid @enderror"
                        placeholder="Isi pilihan A" value="{{ old('option_a', $question->option_a) }}">
                    @error('option_a')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="option_b" class="form-control @error('option_b') is-invalid @enderror"
                        placeholder="Isi pilihan B" value="{{ old('option_b', $question->option_b) }}">
                    @error('option_b')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="option_c" class="form-control @error('option_c') is-invalid @enderror"
                        placeholder="Isi pilihan C" value="{{ old('option_c', $question->option_c) }}">
                    @error('option_c')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="option_d" class="form-control @error('option_d') is-invalid @enderror"
                        placeholder="Isi pilihan D" value="{{ old('option_d', $question->option_d) }}">
                    @error('option_d')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
