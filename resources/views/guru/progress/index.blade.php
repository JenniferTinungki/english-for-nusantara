...
<div class="monitor-page">
    ...
    <div class="filter-card">
        <form method="GET" action="{{ route('guru.progress.index') }}">
            ...
            <a href="{{ route('guru.progress.index') }}" class="btn-reset">
    ...
    <div class="main-card">
        <div class="main-card-header">
            ...
            <span class="result-count">{{ $tableData->count() }} siswa ditampilkan</span>
        </div>
        ...
        @else
            @foreach($tableData as $row)
                ...
                <div class="siswa-block">
                    {{-- Siswa Header (accordion toggle) --}}
                    <div class="siswa-header-row"
                         onclick="toggleDetail('siswa-{{ $siswaId }}', this)"
                         id="hdr-{{ $siswaId }}">
                        ...
                        <div class="siswa-info">
                            ...
                            <div>
                                <div class="siswa-name">{{ $row['siswa']->name }}</div>
                                <div class="siswa-nis">
                                    {{ $row['siswa']->nis ? 'NIS: '.$row['siswa']->nis : $row['siswa']->email }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="siswa-right">
                            <div class="siswa-stats">
                                <div class="stat-item">
                                    <div class="stat-val">{{ $row['opened_count'] }}</div>
                                    <div class="stat-lbl">Dibuka</div>
                                </div>
                                <div class="stat-divider"></div>
                                <div class="stat-item">
                                    <div class="stat-val green">{{ $row['completed_count'] }}</div>
                            ...
                            <div class="mini-progress-wrap">
                                <div class="mini-bar">
                                    <div class="mini-bar-fill"
                                         style="width:{{ $avg }}%; background:{{ $barColor }};"></div>
                                </div>
                                <span class="mini-pct">{{ $avg }}%</span>
                            </div>

                            <div class="toggle-btn" id="icon-{{ $siswaId }}">▾</div>
                        </div>
                    </div>

                    {{-- Detail Materi (collapsible) --}}
                    <div class="siswa-detail" id="siswa-{{ $siswaId }}">
                        <div class="inner-table-wrap">
                            <table class="inner-table">
                                ...
                                <tbody>
                                    @foreach($row['materi_progress'] as $mp)
                                        @php
                                            $pct = $mp['percent'];
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span class="bab-badge">{{ $mp['bab'] }}</span>
                                            </td>
                                            <td>
                                                <div class="materi-title-text">{{ $mp['judul'] }}</div>
                                                @if($mp['subjudul'])
                                                    <div class="materi-sub-text">{{ $mp['subjudul'] }}</div>
                                                @endif
                                            </td>
                                            <td>
                                                <span class="durasi-text">{{ $mp['durasi'] }} mnt</span>
                                            </td>
                                            <td>
                                                @if($pct == 100)
                                                ...
                                                <div class="prog-wrap">
                                                    <div class="prog-bar-outer">
                                                        <div class="prog-bar-inner"
                                                             style="width:{{ $pct }}%; background:{{ $pbColor }};"></div>
                                                    </div>
                                                    <div class="prog-pct">{{ $pct }}%</div>
                                                </div>
                                            </td>
                                            <td>
                                                @if($mp['last_accessed_at'])
                                                    <div class="date-main">{{ \Carbon\Carbon::parse($mp['last_accessed_at'])->format('d M Y') }}</div>
                                                    <div class="date-sub">{{ \Carbon\Carbon::parse($mp['last_accessed_at'])->format('H:i') }} WIB</div>
                                                @else
                                            ...
                                            <td>
                                                @if($mp['completed_at'])
                                                    <div class="date-main">{{ \Carbon\Carbon::parse($mp['completed_at'])->format('d M Y') }}</div>
                                                    <div class="date-sub">{{ \Carbon\Carbon::parse($mp['completed_at'])->format('H:i') }} WIB</div>
                                                @else
                                                    <span style="color:#cbd5e1; font-size:13px;">—</span>
                                                @endif
                                            </td>
