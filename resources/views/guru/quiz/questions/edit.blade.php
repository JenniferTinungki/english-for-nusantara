...
<div class="page-shell">
    <div class="glass-card-strong p-5 mx-auto" style="max-width:1000px;">
        ...
        <form action="{{ route('guru.quiz.questions.update', [$quiz->id, $question->id]) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label class="form-label fw-semibold">Pertanyaan</label>
                <textarea name="question" rows="4" class="form-control" required>{{ old('question', $question->question) }}</textarea>
                @error('question')
