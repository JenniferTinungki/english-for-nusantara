        ...
        @if($quizzes->count() > 0)
            <div class="quiz-grid">
                @foreach($quizzes as $quiz)
                    ...
                    <div class="quiz-card">
                        <div class="quiz-cover" style="background: {{ $cover }};">
                            <div class="quiz-cover-content">
                                <div class="chapter-pill">Chapter {{ $quiz->chapter ?? '-' }}</div>
                                <h3 class="cover-title">{{ $quiz->display_title }}</h3>
                            </div>
                        </div>

                        <div class="quiz-body">
                            <div class="quiz-title">{{ $quiz->display_title }}</div>
                            <div class="quiz-desc">
                                {{ \Illuminate\Support\Str::limit($quiz->display_description, 65) }}
                            </div>

                            <div class="meta-row">
                                <div class="meta-box">
                                    <span class="meta-label">Jumlah Soal</span>
                                    <span class="meta-value">{{ $quiz->questions_count }}</span>
                                </div>
                            ...
                            @if($latestAttempt)
                                <div class="score-badge">
                                    Skor terakhir: {{ $latestAttempt->display_score }}
                                </div>
                            @endif

                            <div class="d-grid">
                                <a href="{{ route('siswa.quiz.show', $quiz->id) }}" class="btn btn-primary btn-premium">
                                    Mulai Quiz
                                </a>
                            </div>
                        </div>
