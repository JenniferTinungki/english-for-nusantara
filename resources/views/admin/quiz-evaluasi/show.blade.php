...
@section('content')
    ...
    <div class="rounded-3xl bg-gradient-to-r from-blue-600 to-indigo-600 p-6 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-lg shadow-blue-200">
        <div>
            ...
            <h1 class="text-2xl font-extrabold text-white tracking-tight">{{ $quiz->judul }}</h1>
            <p class="text-blue-100 text-sm mt-1">Chapter {{ $quiz->chapter ?? '-' }} • {{ $quiz->questions->count() }} soal</p>
        </div>
        <a href="{{ route('admin.quiz-evaluasi.leaderboard', ['quiz_id' => $quiz->id]) }}"
    ...
    {{-- STAT CARDS --}}
    @php $pctLulus = $attempts->total() > 0 ? round(($jumlahLulus / $attempts->total()) * 100) : 0; @endphp
    <div class="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-3xl font-extrabold text-blue-600 leading-none mt-1">{{ $totalPeserta }}</p>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center text-xl flex-shrink-0
                {{ ($rataRata ?? 0) >= 75 ? 'bg-green-100 text-green-600' : (($rataRata ?? 0) >= 50 ? 'bg-yellow-100 text-yellow-600' : 'bg-red-100 text-red-500') }}">
            ...
            <div>
                ...
                <p class="text-3xl font-extrabold leading-none mt-1
                    {{ ($rataRata ?? 0) >= 75 ? 'text-green-600' : (($rataRata ?? 0) >= 50 ? 'text-yellow-500' : 'text-red-500') }}">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-3xl font-extrabold text-green-600 leading-none mt-1">{{ $nilaiTertinggi ?? '-' }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-3xl font-extrabold text-emerald-600 leading-none mt-1">{{ $jumlahLulus }}</p>
                <p class="text-xs text-slate-400 mt-0.5">{{ $pctLulus }}% dari total attempt</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 mb-6">
        ...
        <form method="GET" action="{{ route('admin.quiz-evaluasi.show', $quiz->id) }}"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <span class="text-xs text-slate-400 font-semibold">{{ $attempts->total() }} attempt</span>
        </div>
