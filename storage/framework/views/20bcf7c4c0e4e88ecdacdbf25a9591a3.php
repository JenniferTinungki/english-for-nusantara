...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-extrabold text-slate-800"><?php echo e($totalMateri); ?></p>
                <p class="text-xs text-slate-500 font-medium">Total Materi</p>
            </div>
        </div>

        <?php $__currentLoopData = range(0, 2); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ch): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex items-center gap-4">
            <?php
                $colors = ['purple', 'green', 'orange'];
                $color = $colors[$ch];
            ?>
            <div class="w-12 h-12 rounded-2xl bg-<?php echo e($color); ?>-100 text-<?php echo e($color); ?>-600 flex items-center justify-center text-xl font-bold">
                <?php echo e($ch); ?>

            </div>
            <div>
                <p class="text-2xl font-extrabold text-slate-800"><?php echo e($chapters->get($ch, collect())->count()); ?></p>
                <p class="text-xs text-slate-500 font-medium">Chapter <?php echo e($ch); ?></p>
            </div>
        </div>
    ...
    <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        ...
        <form method="GET" class="flex flex-col md:flex-row gap-4">
            ...
            <div class="w-full md:w-56 relative">
                <select name="bab" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none appearance-none cursor-pointer">
                    ...
                    <?php $__currentLoopData = range(0, 5); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ch): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($ch); ?>" <?php echo e(request('bab') == $ch ? 'selected' : ''); ?>>Chapter <?php echo e($ch); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            ...
            <div class="flex gap-2">
                ...
                <?php if(request('search') || request('bab') !== null && request('bab') !== ''): ?>
                    <a href="<?php echo e(route('admin.materi.index')); ?>" class="px-6 py-3 bg-slate-100 text-slate-600 font-semibold rounded-2xl hover:bg-slate-200 transition-all flex items-center">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <span class="text-xs text-slate-400 bg-slate-100 px-3 py-1 rounded-full"><?php echo e($materi->count()); ?> materi</span>
        </div>
                        ...
                        <tr class="hover:bg-blue-50/40 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="font-bold text-slate-800"><?php echo e($item->judul); ?></div>
                                        <?php if($item->deskripsi): ?>
                                            <div class="text-xs text-slate-500 line-clamp-1"><?php echo e(\Illuminate\Support\Str::limit($item->deskripsi, 45)); ?></div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center">
                                ...
                                ?>
                                <span class="inline-block px-3 py-1 text-xs font-bold rounded-full border <?php echo e($colorClass); ?>">
                                    Chapter <?php echo e($item->bab); ?>

                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-slate-600 font-medium"><?php echo e($item->subjudul ?? $item->bab); ?></span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form action="<?php echo e(route('admin.materi.destroy', $item->id)); ?>" method="POST"
                    ...
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center">
                                ...
                                <a href="<?php echo e(route('admin.materi.create')); ?>" class="inline-flex items-center mt-4 px-5 py-2 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 transition-all">
                                    <i class="fa-solid fa-plus mr-2"></i> Tambah Materi
                                </a>
                            </td>
                        </tr>
        ...
        <?php if(method_exists($materi, 'links') && $materi->hasPages()): ?>
            <div class="p-6 border-t border-slate-100 bg-slate-50">
                <?php echo e($materi->withQueryString()->links()); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\USER\Documents\proyek-pertama-saya\resources\views\admin\materi\index.blade.php ENDPATH**/ ?>